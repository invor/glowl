...
TextureCubemapArray::TextureCubemapArray(std::string id,
...
bool TextureCubemapArray::reload(unsigned int width, unsigned int height, unsigned int layers, GLvoid * data, bool generateMipmap)
{
	m_width = width;
	m_height = height;
	m_layers = layers;
...
void TextureCubemapArray::bindTexture() const
{
	glBindTexture(GL_TEXTURE_CUBE_MAP_ARRAY, m_name);
}
...
unsigned int TextureCubemapArray::getWidth() const
{
	return m_width;
}

unsigned int TextureCubemapArray::getHeigth() const
{
	return m_height;
}

unsigned int TextureCubemapArray::getLayers() const
{
	return m_layers;
}